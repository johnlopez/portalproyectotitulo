...
$.fn.themeswitcher = function(settings){
	...
	/* Theme Loading
	---------------------------------------------------------------------*/
	switcherpane.find('a').click(function(event, trigger){
		...
		if(trigger === undefined) options.onSelect(themeName);
	...
	var button_default = {
		...
		fontSize: '11px',
		color: '#666',
		background: '#eee url(' + wroot + 'img/themeGallery/buttonbg.png) 50% 50% repeat-x',
	...
	.find('.jquery-ui-themeswitcher-icon').css({
		...
		width: '16px',
		height: '16px',
		background: 'url(' + wroot + 'img/themeGallery/icon_color_arrow.gif) 50% 50% no-repeat'
	});	
	//pane css
	switcherpane.css({
		position: 'absolute',
	...
	.find('li').hover(
		function(){ 
			$(this).css({
				'borderColor':'#555',
				'background': 'url(' + wroot + 'img/themeGallery/menuhoverbg.png) 50% 50% repeat-x',
				cursor: 'pointer'
			}); 
		},
		function(){ 
	...
	if( $.cookie(options.cookieName) || options.loadTheme ){
		...
		switcherpane.find('a:contains('+ themeName +')').trigger('click', [true]);
	}
